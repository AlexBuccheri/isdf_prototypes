...
from isdf_prototypes.interpolation_points import (
    assign_points_to_centroids,
    is_subgrid_on_grid, update_centroids, points_are_converged, weighted_kmeans, kmeans_seeding
)
...
def grid2d_shape3():
    """
    Return a grid of the shape (Nx, Ny, Ndim)
    required by multivariate_normal.pdf
    :return:
    """
...
@pytest.fixture()
def gaussians_on_grid2d(grid2d_shape3):
    """ Specific Gaussian distribution
    """
    grid = grid2d_shape3
    assert np.size(grid.shape) == 3
...
@pytest.fixture()
def test_weighted_kmeans(grid2d_shape3, gaussians_on_grid2d):
    # Reshape Gaussians into required shape for weights
    weights = gaussians_on_grid2d.reshape(-1)
    assert weights.shape == (1600,)

    # Reshape grid into required shape
    grid = grid2d_shape3.reshape(-1, 2)
    assert grid.shape == (1600, 2)

    # Number of interpolation points
    n_inter = 48

    # random_indices = np.sort(np.random.choice(grid.shape[0], size=n_inter))
    ...
    initial_centroids = grid[random_indices, :]

    # Perform a single iteration and confirm the initial_centroids returned never change
    centroids, iter = weighted_kmeans(grid, weights, initial_centroids,
    ...
    # With the random assign at equidistance removed,
    centroids, iter = weighted_kmeans(grid, weights, initial_centroids,
                                      n_iter=100, centroid_tol=1.0e-9,
                                      safe_mode=True, verbose=False)

    assert iter == 18, ("For a fixed seed of centroids, the algorithm converges to the same interpolation points each "
                        "time.")
    ...
    assert np.allclose(centroids,
                       ref_centroids), "For a fixed seed of centroids, the algorithm converges to the same interpolation points each time."


def test_kmeans_seeding(grid2d_shape3):
    # Reshape grid into required shape
    grid = grid2d_shape3.reshape(-1, 2)
    assert grid.shape == (1600, 2)
    ...
    # Probably going to need to wrap `np.random.choice` such that it takes the loop index.
    # Then I can mock it


    optimal_centroids = kmeans_seeding(grid, 2)
